# Documentation Guide

This directory contains the public documentation for OakIdeas.Aspire.DataExplorer.

## Getting started

- [Local development setup](setup/local-development.md)
- [Usage quickstart](usage/quickstart.md)
- [Sample application walkthrough](samples/README.md)

## Architecture

- [Architecture overview](architecture/overview.md)
- [Development-only boundary](architecture/development-only-boundary.md)
- [Metadata discovery](architecture/metadata-discovery.md)
- [Provider model](architecture/provider-model.md)
- [Error handling](architecture/error-handling.md)
- [Provider implementation guide](providers/implementation-guide.md)
- [SQL Server provider notes](providers/sqlserver.md)

## Packaging and publishing

- [NuGet package README source](nuget/package-readme.md)
- [Contributor package guide](publishing/contributor-package-guide.md)
- [Package publishing guide](publishing/package-publishing-guide.md)

## Decisions and troubleshooting

- [Architecture decisions](decisions/)
- [Troubleshooting common errors](troubleshooting/error-handling.md)
